...
type TooltipInformation =
  { Signature: string
    Summary: string
    Parameter: string }
  static member Empty = { Signature = ""; Summary = ""; Parameter = "" }
...
module SymbolTooltips =
    ...
    let syntaxHighlight s = s
    let asUnderline = sprintf "_STARTUNDERLINE_%s_ENDUNDERLINE_"
    
    type XmlDoc =
        | Full of summary: string
        | Lookup of signature: string * file: string option
        | EmptyDoc
    ...
    let getKeywordTooltip (keyword:string) =
        ...
        let summary =
            ...
            | true, description -> Some description
            | false, _ -> None
...
//            |> Seq.iter(fun symbol -> print sb symbol)
    ...
    let footerForType (entity:FSharpSymbolUse) =
        ...
        | MemberFunctionOrValue m ->
            if m.FullType.HasTypeDefinition then
                ...
                sprintf "From %s:\t`%s`%sAssembly:\t%s" parentDesc parentType Environment.NewLine ent.Assembly.SimpleName
            else
                sprintf "Assembly:\t%s" m.Assembly.SimpleName
      
        | Entity c ->
            let ns = c.Namespace |> Option.getOrElseFun (fun () -> c.AccessPath)
            let fullName =
                match c.TryGetFullNameWithUnderScoreTypes() with
                | Some fullname -> "Full name: " + fullname
                | None -> "Full name: " + c.QualifiedName

            sprintf "%s%sNamespace:\t%s%sAssembly:\t%s" fullName Environment.NewLine ns Environment.NewLine c.Assembly.SimpleName
      
        | Field f ->
            let parent = f.DeclaringEntity.UnAnnotate().DisplayName
            sprintf "From type:\t`%s`%sAssembly:\t%s" parent Environment.NewLine f.Assembly.SimpleName
        ...
        |  UnionCase uc ->
            ...
            sprintf "From type:\t`%s`%sAssembly:\t%s" parent Environment.NewLine uc.Assembly.SimpleName
        | _ -> ""
    ...
    let getTooltipInformationFromTip tip =
      async {
            try
                ...
                let toolTipInfo = { Signature = signature; Summary = footer; Parameter = "" }
                let result =
                  match xmldoc with
                  | Full summary -> { toolTipInfo with Summary = summary }
                  | Lookup(key, potentialFilename) ->
                      let summary =
                        maybe { let! filename = potentialFilename
                                let! markup = TooltipXmlDoc.findDocForEntity(filename, key)
                                ...
                                return summary }
                      match summary with
                      | Some summary -> { toolTipInfo with Summary = summary }
                      | None -> toolTipInfo
                  | EmptyDoc -> toolTipInfo
                return result
            with _ ->
                //LoggingService.LogError ("F# Tooltip error", ex)
                return TooltipInformation.Empty }
    ...
    let getTooltipInformation symbol =
        async {
            ...
            | _ -> return TooltipInformation.Empty
        }
    ...
    let getTooltipInformationFromSignature summary signature parameterName =
        ...
        if not (String.isNullOrEmpty parameterInfo) then
            { toolTipInfo with Parameter = parameterInfo }
        else
            toolTipInfo
